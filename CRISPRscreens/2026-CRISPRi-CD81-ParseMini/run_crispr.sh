...
GUIDES="sgRNA_CD81.csv"
KIT="WT_mini"
CHEM="v3"

# Loop over all CRISPR sublibraries present
for SUB in $(ls BM076_CRISPR_Sublibrary_*_R1_001.fastq.gz | sed 's/.*Sublibrary_\([0-9][0-9]\).*/\1/' | sort -u)
do
    echo "Processing CRISPR Sublibrary $SUB"

    # Collect all R1 and R2 files for this CRISPR sublibrary
    FQ1=$(ls BM076_CRISPR_Sublibrary_${SUB}_*_R1_001.fastq.gz | tr '\n' ' ')
    FQ2=$(ls BM076_CRISPR_Sublibrary_${SUB}_*_R2_001.fastq.gz | tr '\n' ' ')
