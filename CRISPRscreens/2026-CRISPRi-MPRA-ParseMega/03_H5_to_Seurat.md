...
wta.data <- open_matrix_10x_hdf5(path = "output_combined_filtered/count_matrix.h5")

# Write the matrix to a directory
write_matrix_dir(mat = wta.data, dir = 'output_combined_filtered/bpcells')

# Now that we have the matrix on disk, we can load it
wta.mat <- BPCells::open_matrix_dir(dir = "output_combined_filtered/bpcells")
metadata <- read.csv("output_combined_filtered/cell_metadata_sub.csv", head=T)
...
# Save file
saveRDS(scobj, file="output_combined_filtered/scobj.rds")
```